...
internal sealed class WindowManagerHost
{
    ...
    public int Run(string[] args)
    {
        ...
        _log.Info("window_manager_started", new { mode = "foundation" });
        LoadSettings();
        ...
        _log.Info("window_manager_stopped", new { reason = "host_completed" });
        return 0;
    }
    ...
    private void RunManageMode()
    {
        ...
        using var server = new WindowManagerServer(this, _log);
        ConsoleCancelEventHandler cancelHandler = (_, eventArgs) =>
        {
            eventArgs.Cancel = true;
            eventSource.Stop();
        ...
        try
        {
            _requestShutdown = eventSource.Stop;
            _scheduleEligibilityRecheck = handle => eventSource.ScheduleRecheck(handle, EligibilityRecheckDelay);
            eventSource.Start(HandleWindowEvent);
            try
            {
            ...
            server.Start();
            _log.Info("management_started", new { pipeName = server.PipeName });
            SystemEvents.SessionSwitch += OnSessionSwitch;
            eventSource.RunMessageLoop((messageId, _) =>
            {
                if (messageId is NativeMethods.WmDisplayChange or NativeMethods.WmSettingChange
    ...
    public WindowManagerResponse HandleRequest(WindowManagerRequest request)
    {
        try
        {
            switch (request.Command)
            {
                case WindowManagerCommand.ExecuteCommand:
                    if (request.TilingCommand is null)
                    {
                        return new WindowManagerResponse(false, "missing_tiling_command");
                    }

                    ExecuteTilingCommand(request.TilingCommand.Value);
                    return new WindowManagerResponse(true);

                case WindowManagerCommand.SetMouseFocusSuspended:
                ...
                default:
                    return new WindowManagerResponse(false, "unknown_command");
            }
        }
        catch (Exception exception)
        {
            _log.Error("request_failed", new { command = request.Command.ToString(), exception = exception.GetType().Name });
            return new WindowManagerResponse(false, exception.GetType().Name);
        }
    }

    private WindowManagerStatus BuildStatus()
    {
        ...
        var monitors = _monitorCatalog.GetWorkAreas()
            .Select(monitor => new MonitorBarState(
                monitor.Handle.ToInt64(),
                _workspaceState.GetCurrentWorkspace(monitor.Handle),
                _layoutState.Get(_workspaceState.GetCurrentWorkspace(monitor.Handle), monitor.Handle, _layoutOptions).Mode.ToString()))
            .ToArray();
        return new WindowManagerStatus(
            _managementState.IsEnabled,
            currentWorkspace,
    ...
    {
        if (_mouseFocusSuspended
            || !_settings.FocusFollowsMouse
            || !_managementState.IsEnabled
    ...
    private static bool ShouldRetileFor(WindowEventKind kind)
    {
